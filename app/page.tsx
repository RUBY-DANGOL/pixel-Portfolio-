...
export default function Page() {
  ...
  return (
    <div className="text-stone-600 font-pixel min-h-screen overflow-x-hidden scroll-smooth">
      <section className="min-h-screen bg-cover bg-center relative pb-28 pt-4 px-6 sm:px-12 md:px-24 py-8" >
        ...
        <main className="grid grid-cols-1   md:grid-cols-5 items-center px-6 sm:px-12 md:px-24 lg:px-32 min-h-[80vh] relative pt-10 sm:pt-0 gap-10">
          <section className="text-left text-[clamp(1rem,8vw,4rem)] tracking-tighter font-black max-w-2xl w-full col-span-3">
            <p className="leading-none">A Journey</p>
            <p className="leading-none">of</p>
            <p className="leading-none">Curiosity</p>
            <p className="mt-6 text-sm leading-snug font-normal not-italic tracking-normal">
              ...
              <span className="text-sm text-stone-400 font-normal not-italic">There are no experts. There's only us.</span>
              <div className="flex justify-start sm:justify-start mt-2">
                <a href="/Rubina-Dangol.pdf" download className="w-64 h-10 flex items-center justify-center bg-rose-100/60 text-stone-600 border-4 border-gray-300 shadow-[4px_4px_0_0_black] hover:shadow-[2px_2px_0_0_black] active:shadow-none transition-all font-bold text-s">
                  Download Resume
              </a>
              </div>
            </p>
...
/>
            ...
            <div className="mt-4 w-full flex flex-col items-center space-y-4">
              <audio ref={audioRef} src="/song.mp3" preload="auto" loop onTimeUpdate={handleTimeUpdate} onLoadedMetadata={() => { if (audioRef.current) setDuration(audioRef.current.duration) }} />
